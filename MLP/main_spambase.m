function [] = main_spambase()
    ...
    for j=1:length(epochs)
        ...
        accuracies = zeros(length(neurons),numel(fileNames)*numExecutions*foldCount);
        cellEpochsExecuted = epochsExecuted;
        epochsExecuted = zeros(length(neurons),numel(fileNames)*numExecutions*foldCount);
        for i=1:length(neurons)
